...
function dialogue(_i_dialogue_number, _bool)
{
	if (global.i_selected_message != 0)
	{
		instance_create_layer(x + 200, 1300, "Chat", obj_response, {image_index : _i_dialogue_number + b_dialogue_offset + global.i_selected_message - 1});
...
function selectbutton(_i, _b_multiple_dialogue)
{
	if (!global.b_selecting_message)
	{
		...
		{
			instance_create_layer(1650, 600, "instances", obj_dialogue, {image_index : _i + b_dialogue_offset + 1});		
		}
	}
	global.b_selecting_message = true;
	dialogue(_i, _b_multiple_dialogue);	
}